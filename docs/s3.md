...
Second tier storage is used for storing data and files that you don't need readily accessible on tier 1. Prior to summer 2024, each user was granted up to 120 TB of storage on tier 2. MSI has been in the process of updating their policies around this so now new users are limited to 5 GB, with their documentation stating that [MSI *groups* are now limited to 120 TB](https://msi.umn.edu/about-msi-services/data-storage/second-tier-storage). We're working on clarifying the details on this and will update the documentation once we know more. **In the meantime, using tier2 storage whenever possible is highly recommended in order to prevent storage quota issues on tier1.**

For a comprehensive introduction to s3, see [this tutorial video](https://drive.google.com/drive/folders/1Oz3i5lbld5VmXGdhWagOMWYOIJmgrJA4) (recorded as part of the Data Processing Workshop hosted for undergraduate research assistants). It is encouraged to watch this tutorial prior to your first time using s3 commands. 
...
`s3cmd du -H s3://bucket-name/` -check the size of a single bucket (when using commands like `du` or `ls` on large buckets, we recommend using [s5cmd client](#s5cmd-client)<br>
