...
Second tier storage is used for storing data and files that you don't need readily accessible on tier 1. Prior to summer 2024, each user was granted up to 120 TB of storage on tier 2. MSI has been in the process of updating their policies around this so now new users appear to be limited to 5 GB, with their documentation stating that [MSI *groups* are now limited to 120 TB](https://msi.umn.edu/about-msi-services/data-storage/second-tier-storage). We're working on clarifying the details on this and will update the documentation once we know more. **In the meantime, using tier2 storage whenever possible is highly recommended in order to prevent storage quota issues on tier1.**
