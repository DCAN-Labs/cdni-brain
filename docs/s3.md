...
For a comprehensive introduction to s3, see [this s3 tutorial video](https://drive.google.com/drive/folders/1Oz3i5lbld5VmXGdhWagOMWYOIJmgrJA4) (recorded as part of the Data Processing Workshop hosted for undergraduate research assistants). It is encouraged to watch this tutorial prior to your first time using s3 commands. If you don't have access to the CDNI google drive to view the video, ask Kim or Luci to be added.

## S3 Basics and Usage

It's important to know that S3 is not actually a filesystem, but an object storage system (See: [AWS documentation explaining storage types](https://aws.amazon.com/compare/the-difference-between-block-file-object-storage/)), and S3cmd is an [API tool](https://www.geeksforgeeks.org/what-is-an-api/) for object storage. As a result, the logic you are likely more familiar with when interacting with folder structures on the command line doesn't necessarily translate to s3cmd. S3 buckets don't actually have real directory structures, so when referencing a specific file or folder on S3, you provide what may *look* like a file path, but is actually a *key prefix* used to iterate through objects with matching substrings. This means you cannot `cd` into a s3 bucket, but instead have to continuously `ls` a bucket to see what is inside each subfolder.  
...
When the trailing `/` is present, s3cmd iteratates through the bucket objects to list objects containing `luci-scratch/ABCC/*` and returns the following:
```
$ s3cmd ls s3://luci-scratch/ABCC/
...
Note that this logic also means that wildcard (`*`) doesn't work for most s3 commands

### S3cmd Usage 

Read [s3cmd usage](https://s3tools.org/usage) for full usage

`s3info -u x500` - view your s3 credentials and tier2 disk usage<br> 
`s3cmd du -H s3://bucket-name/` - check the size of a single bucket (when using commands like `du` or `ls` on large buckets, we recommend using the [s5cmd client](#Using-s5cmd))<br>
`s3cmd ls` - list all buckets you are owner on (buckets that you do not own, but have been given access to, will not appear in this list)<br>
`s3cmd rm s3://bucket-name/file` - remove a file from a bucket<br>
`s3cmd rb s3://bucket-name` - remove an entire bucket<br>
...
#### Making a new bucket 
`s3cmd mb s3://bucket-name/` - create a new bucket 
You're welcome to create your own buckets for personal use, but if you were added to MSI after summer 2024, you only have 5GB of tier 2 space. 

<div class="admonition attention">
	<p class="first admonition-title">Attention</p>
	<p class="last">
		Buckets intended to store large amounts of data or data repositories should be created by a PI.
	</p>
</div>

This is for two reasons: PIs and legacy users have a larger storage quota on tier2 (120TB) and if you leave UMN, others will lose access to your bucket once your UMN account is deactivated!!

Also note that, while this has not always been the case, MSI currently requires users to follow AmazonS3 naming standards, e.g. names cannot include underscores or uppercase letters - see the [AmazonS3 bucket naming rules](https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html) for full details.
...
export PATH=$PATH:/projects/standard/faird/shared/code/external/utilities/s5cmd
...
To gain access to a bucket, ask the bucket owner or others with access granted. If you're unsure of who already has access, consult the tier2 tab of CDNI's [data tracking spreadsheet](https://docs.google.com/spreadsheets/d/1QpKYJQqhuxoQhErBscAEev9npsd1RgKS8KdCL6FiuEo/edit?usp=sharing). Note that none of the scripts or commands below will work if you don't have bucket access. 

We generally use fairly permissive s3 policies and so typically anyone with access to a bucket (not the bucket owner alone) can grant access to a new user. MSI provides instructions for how to [grant someone access to a bucket](https://www.msi.umn.edu/support/faq/how-do-i-use-s3-buckets-share-data-tier-2-storage-other-users) using an s3 policy file, but the simplest method is to use the `set_s3policy` command with the following steps:  

1. If you haven't already done so, add the line `export PATH="/projects/standard/faird/shared/code/internal/utilities/s3policy_bin/:$PATH"` to your `~/.bashrc`and open a new terminal after saving your changes in order for your changes to take effect.
2. Print list of current users with bucket access with the command<br> `/projects/standard/faird/shared/code/internal/utilities/MSI-utilities/s3_get_x500/get_x500.sh s3://bucket_name/`
3. Copy the *FULL LIST* (see **NOTE** below), append the x500 for the new user you wish to grant access (x500_3 in the example below), and feed into the `set_s3policy` command like so, excluding the s3:// prefix for the bucket name in the command:<br> `set_s3policy x500_1,x500_2,x500_3 bucket_name FULL`
...
**NOTE: You must copy the full list from `get_x500.sh` because every time you set a policy you need to list EVERYONE that needs access. You CANNOT just list that one person's x500: this will result in only that person having bucket access and access removal for all other x500s, including the bucket owner. `get_x500.sh` will also list your x500 since you need to have access to the bucket in order to run this command.**

If you do not have access to the `faird` share, you can create your own `get_x500.sh` script with the following code: 

```
# Give full path to bucket (ex: s3://bucket-name/)
bucket=$1

x500=$( for uid in $(s3cmd info ${bucket} | grep -oE '([0-9]{5})'); do s3info -u ${uid} | grep 'Sharing' | cut -d \@ -f 1 | cut -d \: -f 2; done )
...
## Advanced S3 Policy usage

While `set_s3policy` should be sufficient for most uses, for more fine-grained management of bucket permissions you can manually create a bucket policy JSON file and apply it with `s3cmd setpolicy FILE s3://BUCKET`

See the [AWS S3 documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/example-bucket-policies.html) for examples of bucket policy files. 
...
You shouldn't have to make any changes to the file unless the s3 bucket is hosted elsewhere than *s3.amazonaws.com* (e.g. *s3.msi.umn.edu* for MSI). You can test your access through an `ls` command - the `s3cmd -c ~/.s3cfg-RANDOM` will have to be part of every command you run to access the buckets that require the credentials you provided.

## S3 access with Cyberduck 
[View your MSI S3 credentials (login required)](https://mymsi.msi.umn.edu/user/second-tier-storage) 

[Cyberduck S3 documentation](https://docs.cyberduck.io/protocols/s3/)
